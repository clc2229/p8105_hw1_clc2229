...
hw_df_2 = tibble(
  ...
  y = rnorm(1000),
  xy_positive = (x + y) > 2,
  numeric_logical = as.numeric(xy_positive),
  factor_logical = as.factor(xy_positive)
)
```
  
The dataframe created by the above code chunk has a size of: rows = `r nrow(hw_df_2)`, columns = `r ncol(hw_df_2)`. The mean of x is `r round(mean(hw_df_2$x), digits = 2)`. The median of x is `r round(mean(hw_df_2$x), digits = 2)`. The proportion of cases for which the logical vector is TRUE is `r mean(hw_df_2$xy_positive)`.
